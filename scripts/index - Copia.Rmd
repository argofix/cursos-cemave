...
# renderLeaflet({
#   
# mapa_cursos <- leaflet() %>% addTiles() %>% addCircleMarkers(data=mapa, lat = ~latitude, lng = ~longitude, radius = ~3, color = "#00240B", popup = ~popup_info) # inserindo a janela popup no código
# 
# })
#   
# tabela <- subset(cursos, select = -c(ID, categoria, latitude, longitude, instrutores.origem, programa.curso, lista.participantes, relatorio.descritivo, fotos, origem.informacoes, observacoes))
# 
# tabela_cursos <- datatable(tabela, extensions="Scroller", style="bootstrap", class="compact", width="100%", options=list(deferRender=TRUE, scrollY=300, scroller=TRUE))
# 
# 
# # Crosstalk
# 
# shared_mapa <- SharedData$new(mapa)
# 
# renderLeaflet({
# 
# shared_mapa <- shared_mapa %>% leaflet() %>% addTiles() %>% addCircleMarkers(data=mapa, lat = ~latitude, lng = ~longitude, radius = ~3, color = "#00240B", popup = ~popup_info) # inserindo a janela popup no código
# 
# })
# 
# 
# shared_tabela_cursos <- tabela_cursos <- datatable(tabela, extensions="Scroller", style="bootstrap", class="compact", width="100%", options=list(deferRender=TRUE, scrollY=300, scroller=TRUE))
...
# filter_slider("CH", "Carga Horária", mapa, column=~`carga.horaria`, step=8)
# filter_checkbox("mod", "Modalidade do curso", mapa, ~`modalidade`, inline = F)
...
# bscols(shared_mapa, shared_tabela_cursos)
...
Este relatório apresenta os resultados do processo de compilação, sistematização e análise de dados relativos aos cursos ministrados pelo Centro Nacional de Pesquisa e Conservação de Aves Silvestres – CEMAVE no período de 1978 a 2020. A iniciativa teve como objetivos sistematizar o esforço de capacitação realizado pelo Centro ao longo de sua existência e disponibilizar essa informação ao público em geral. Como nem todas as informações consideradas foram obtidas para todos os cursos compilados (especialmente para os cursos mais antigos, apenas informações parciais foram obtidas), nos  resultados apresentados neste relatório o número de registros considerados difere entre as variáveis tratadas, a depender do aspecto avaliado. Apesar das lacunas existentes, procurou-se extrair a maior quantidades possível de informações dos dados disponíveis.

Nas primeiras décadas predominaram no CEMAVE os cursos com foco em atividades práticas em campo (p. ex.: capturas com redes de neblina, anilhamento, identificação e registros comportamentais) e que incluiam pequenos projetos em grupo ou individualmente). Os cursos eram, em geral, mais longos (9, 10, 11, até 17 dias). Com o passar dos anos houve uma tendência de redução da duração dos cursos, com uma progressiva convergência para o predomínio de cursos com 5 dias (40 horas – uma semana). Esses cursos, com foco nas atividades práticas de campo e capacitação para o anilhamento estão, até hoje, fortemente associados à imagem institucional do CEMAVE como centro especializado em pesquisa e conservação de aves, nacionalmente conhecido e reconhecido pela comunidade ornitológica do país. As iniciativas de capacitação do Centro estão intrinsicamente ligadas ao seu próprio processo de criação e ao interesse e necessidade existentes, à época, na formação de profissionais para atuar na pesquisa e conservação de aves no Brasil.  

Ao longo dos anos parece ter havido um aumento da demanda para a organização de cursos de menor duração, voltados à capacitação de um público atuante na área de fiscalização/proteção e reabilitação de aves. Essa prática já existia em períodos anteriores, com cursos voltados especialmente aos fiscais do Ibama, órgãos ambientais estaduais e também a profissionais de CETAS, mas parece ter havido um incremento dessa demanda em anos recentes, possivelmente reflexo também de ações previstas nos Planos de Ação Nacionais para a conservação de espécies ameaçadas. Nos últimos anos têm surgido também demandas pontuais no âmbito acadêmico para cursos mais teóricos sobre biologia e conservação de aves e sobre as ações desenvolvidas pelo CEMAVE, incluindo treinamentos para o uso de aves em programas de monitoramento. Esse cenário geral parece ter contribuído para o aumento do número de cursos ministrados no período de 2010 a 2020.

Os cursos ministrados pelo CEMAVE têm uma grande abrangência tanto em termos geográficos quanto em termos da área de atuação dos alunos. Já ministramos cursos em grande parte dos estados brasileiros, alcançando alunos com diferentes perfis de atuação profissional, provenientes de 26 estados de todas as regiões do país, além de alguns alunos de países vizinhos. Atualmente há espaço há espaço para ampliação do número de participantes provenientes de instituições de ensino e pesquisa, seja como cursistas ou como instrutores.
